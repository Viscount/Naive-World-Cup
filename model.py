...
def get_match_fame(squads, code_a, code_b):
    ...
    for player in squads:
        ...
        if player["Nation"] == code_a:
            fame_a += get_player_fame_bonus(player_posts_count) * get_player_fame_lvl(player_fans_count)
        if player["Nation"] == code_b:
            fame_b += get_player_fame_bonus(player_posts_count) * get_player_fame_lvl(player_fans_count)
...
def main():
    ...
    for match in matches:
        ...
        print("unc_score: %.2f goal_score: %.2f fame_score: %.2f" % (uncertainty_score, goal_score, fame_score))
